// roadRouter.mjs — a small routing engine over an OpenStreetMap road
...
// free-flow time. It has no traffic, which is the honest limit of
// speed-limit routing and the reason measured times come next. But the
// water and the bridges are already exact here, because the graph only
// connects where roads actually connect.
...
function parseMaxspeed(tag, highway) {
...
export function buildGraph(elements) {
  const nodeLat = new Map();
  const nodeLng = new Map();
  ...
  function idx(osmId) {
    ...
    if (i === undefined) {
      i = lats.length;
      idToIdx.set(osmId, i);
      lats.push(nodeLat.get(osmId));
      lngs.push(nodeLng.get(osmId));
    }
    return i;
  }
...
export function makeSnapper(graph, mask) {
...
export function dijkstra(graph, source, dist) {
  dist.fill(Infinity);
  dist[source] = 0;
  // Heap of packed (dist, node); node in low 21 bits is enough here but
  // we keep parallel arrays for clarity.
  const heapDist = [0];
  const heapNode = [source];
  const swap = (a, b) => {
...
export function driveMatrix(graph, anchors) {
  const { mask, size } = largestScc(graph);
  process.stderr.write(`road: largest connected component has ${size} of ${graph.n} nodes\n`);
  ...
  for (let i = 0; i < n; i++) {
    ...
    if ((i + 1) % 25 === 0 || i === n - 1) {
      process.stderr.write(`\rroutes: ${i + 1}/${n}`);
    }
  }
